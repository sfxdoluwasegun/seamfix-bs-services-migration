...
public class NotificationCache extends BsClazz {
	...
	@PostConstruct
	public void init(){
		logger.debug("Initializing Cache for Notifications");
		addressList = appProps.getProperty("notifications-cache-server-list", "localhost:11211");
		connect();
	}
	...
	protected void connect(){
		try {
			...
			logger.debug("Connected notifications cache to memcached servers: " + addressList);
		} catch (IOException e) {
			logger.error("", e);
		}
	}
	...
	private boolean validateConnection(){
		if(client == null){
			logger.error("Could not connect to Notification memcache, retrying: " + addressList);
			connect();
		}
		return client != null;
	}
