 ...
 * for biodata caching and retrieval
 * @author Nnanna
 * @since 26/01/2017
 *
 */
...
public class BiodataCache extends BsClazz {
	...
	@PostConstruct
	public void init(){
		logger.debug("Initializing Cache for Biodata");
		addressList = appProps.getProperty("biodata-cache-server-list", "localhost:11211");
		connect();
	}
	...
	protected void connect(){
		try {
			...
			logger.debug("Connected biodata cache to memcached servers: " + addressList);
		} catch (IOException e) {
			logger.error("", e);
		}
	}

	private boolean validateBioCacheConnection(){
		if(client == null){
			logger.error("Biodata memcached server connection was unsuccessful, retrying: " + addressList);
			connect();
		}
		return client != null;
	}
